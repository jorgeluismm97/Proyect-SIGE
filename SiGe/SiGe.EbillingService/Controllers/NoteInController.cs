...
{
    public class NoteInController : Controller
    {
        ...
        public NoteInController(INoteTypeService noteTypeService, INoteService noteService, INoteDetailService noteDetailService, IProductService productService)
        {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var noteViewModel = await _noteService.GetByCompanyIdActionTypeAsync(HttpContext.Session.GetInt32("companyId").Value,0);
        ...
        public async Task<IActionResult> Create(NoteCreateViewModel  noteCreateViewModel)
        {
            if (ModelState.IsValid)
            {
                ...
                foreach (var item in noteCreateViewModel.Details)
                {
                    ...
                    await Task.Run(async () =>
                    { return await _noteDetailService.AddAsync(noteDetailModel); }
);
                }
            ...
            return RedirectToAction("index", "NoteIn");
        }

        public async Task<IActionResult> Details(int id)
        {
            var lstnote = await _noteService.GetByCompanyIdActionTypeAsync(HttpContext.Session.GetInt32("companyId").Value, 0);
